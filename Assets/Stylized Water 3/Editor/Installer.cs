    ...
    public static class Installer
    {
        ...
        public static void Initialize()
        {
            ...
            SetupItem unityVersion = new SetupItem(AssetInfo.VersionChecking.GetUnityVersion());
            {
                ...
                unityVersion.description = $"Compatible and supported Unity version";
                
                //Too old
                if (AssetInfo.VersionChecking.compatibleVersion == false)
                {
                    unityVersion.state = MessageType.Error;
                    unityVersion.description = $"This version of Unity is not compatible and is not subject to support. Update to at least {AssetInfo.MIN_UNITY_VERSION}. Errors and issues need to be resolved locally";
                }
                else
                {
                    //Too broken
                    ...
                    if (AssetInfo.VersionChecking.supportedVersion == false)
                    {
                        unityVersion.state = MessageType.Warning;
            ...
            SetupItem compileErrors = new SetupItem("Script compilation");
            {
                ...
                if (errors > 0)
                {
                    ...
                    compileErrors.description = "Your project has one ore more script compilation errors. These can prevent the water shader from working, or components from being available!";
                }
                else
                {
